...
The output gives several useful metrics that describe how well the data fit the linear regression model.  For the `Page` variable, the `Sum Sq` field in the output above gives the sums of squares from the regression model, or the difference between each page mean and the grand mean (i.e., the mean calculated from all observations combined).  The sums of squares of the residuals is the squared difference between each observation and the `Page` mean.  