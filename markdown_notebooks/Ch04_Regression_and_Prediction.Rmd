...
house_sales_98105_partial_resid <- predict(house_mod_98105, 
        type = "terms") %>% 
  data.frame() %>% 
  rename_with(~ paste(.x, "_prediction", sep = "")) %>% 
  cbind(house_sales_98105) %>%
  add_residuals(model = house_mod_98105)
  
# Isolate SqFtTotLiving and calculate partial resid
house_sales_98105_partial_resid %>% 
  dplyr::select(SqFtTotLiving, resid) %>% 
  mutate(partial_resid = SqFtTotLiving + resid) %>% 
