...
library(modelr)
library(ggcorrplot)
library(moderndive) # for additional regression functions
library(splines) # fitting spline regression models
library(mgcv) # gitting generalized additive models
...
* The standardized residuals are the residuals of a record divided by the standard error. Practically, these can be interpreted as the number of standard errors away from the regression line.

* Below, we've fitted a model with the zipcode 98105, and we'll calculate the standardized residuals using the `rstandard()` function.

```{r housing-model-98105-zipcode}
# Subset the housing data
house_sales_98105 <- house_sales %>% 
  filter(ZipCode == "98105")

# Fit a linear model
house_mod_98105 <- lm(AdjSalePrice ~ SqFtTotLiving + 
                        SqFtLot + Bathrooms + Bedrooms +
                        BldgGrade, 
                      data = house_sales_98105)

# Calculate the standardized residuals
std_residuals <- rstandard(house_mod_98105)

# Look at the standardized residuals
data.frame(std_resid = std_residuals) %>% 
  rownames_to_column("record") %>% 
  ggplot(aes(std_resid)) +
  geom_histogram(bins = 30) +
  my_theme +
  scale_x_continuous(breaks = seq(-5, 5, by = 2.5)) +
  labs(title = "Some records are more than four standard errors from the regression line", 
       x = "Standardized resdiuals", 
       y = "Count")

# Extract the records with the most extreme standardized residuals
std_resid_df <- data.frame(std_resid = std_residuals) %>% 
  rownames_to_column("record")

# Bind the standardized residuals df to the original data
house_sales_98105 <- house_sales_98105 %>% 
  mutate(std_resids = std_resid_df$std_resid) %>% 
  dplyr::select(AdjSalePrice, SqFtTotLiving, SqFtLot, 
         Bathrooms, Bedrooms, BldgGrade, std_resids)

# Filter data to find the most influential observations based on the standardized residuals (> 2.5)
house_sales_98105 %>% 
  filter(std_resids > 2.5) %>% 
  arrange(desc(std_resids))
```

* There is an important difference between an outlier, which will have a large residual (i.e., difference between the predicted value from the regression model and the actual observation) versus a point that has a high leverage, or large influence in the regression model. 

* One of the metrics utilized to evaluate the influence of individual observations is the hat-value.  

* Another metric to evalute the influence of an individual observation is the Cook's distance, which takes into account the leverage and the size of the residual.

* Some of the metrics can be calculated using the `fortify()` function, which will calculate the hat-values and Cook's distance, in addition to extract the fitted values from the regression model.  All of these values will be bound to the original data frame (supplied in the `data` argument).

```{r fortify-extract-data}
# Extract fitted values and calculate model fit statistics
house_sales_98105 <- fortify(house_mod_98105, 
                             data = house_sales_98105)

# Create a bubble plot with the hat values on the x-axis and standardized residuals on the y-axis
house_sales_98105 %>% 
  ggplot(aes(.hat, std_residuals)) +
  geom_point(aes(size = 10*sqrt(.cooksd)), 
             shape = 21) +
...
* The above bubble plot shows the hat-values on the x-axis and the standardized residuals on the y-axis, and the size of the points is related to the Cook's distance.

* [These online statistics notes](https://online.stat.psu.edu/stat462/node/173/) introduce the idea of using Cook's distance, which takes into account both the residuals and the hat-values.  Essentially, the metric describes how the behavior of all of the fitted values change when a particular observation is deleted.

* For example, a point with a large Cook's disatnce indicates that removing that observation from the data set will have a large influence on the behvaior of the remaining fitted values (i.e., it will have a large influence on the regression overall).

* The textbook suggests that an observation is influential if the Cook's distance is greater than $4/(n-P-1)$, where $n$ is the total number of observations, and $P$ is the number of explanatory variables in the model.

```{r cooks-distance-histogram}
...
* We can see that most of the Cook's distances are near 0, but there are several greater than 0.1.  We can compare the coefficients of the original model and a new model with the influential observations removed (i.e., Cook's distance > 0.1).

```{r remove-influential-records}
# Get original house model coefficients into a data frame
house_mod_df <- house_mod_98105 %>% 
  tidy() %>% 
  dplyr::select(term, estimate) %>% 
  rename(all_records_estimate = estimate)

# Remove influential records with Cook's distance > 0.1
house_sales_98105 %>% 
  filter(.cooksd < 0.1) %>% 
  lm(AdjSalePrice ~ SqFtTotLiving + SqFtLot + Bathrooms + 
       Bedrooms + BldgGrade, data = .) %>% 
  broom::tidy() %>% 
  dplyr::select(term, estimate) %>% 
  rename(influential_removed_estimate = estimate) %>% 
  left_join(house_mod_df, by = "term") %>% 
  dplyr::select(term, all_records_estimate, influential_removed_estimate) %>% 
  mutate(across(where(is.numeric), round))
  
```
* The regression coefficients change quite dramatically after the influential observations have been removed, particularly for the `Bathrooms` term.
 
* Another key regression diagnostic we can evaluate is heteroskedascity, which looks at the distribution of the variance across the different predicted values.  Essentially, we are looking for a model in which the variance between the predicted values and the residuals are constant; in a case of a data set which displays heteroskedascity, the variance in residuals will be different across the range of predicted values.

* For example, we can use the `add_predictions()` function to add the predicted values from the housing model to the original data frame, then plot the predicted values on the x-axis versus the absolute value of the residuals on the y-axis.

```{r abs-residuals-vs-predicted}
# Add the predicted values and residuals to the original data frame
house_sales_98105 %>% 
  add_predictions(house_mod_98105) %>% 
  add_residuals(house_mod_98105) %>% 
  ggplot(aes(pred, abs(resid))) + 
  geom_point(alpha = 0.65) +
  geom_smooth(se = TRUE) +
  my_theme +
  labs(x = "Predicted value", 
       y = "Absolute value of the residuals", 
       title = "Heteroskedascity is observed in the housing model in the 98105 zipcode")
...
* We can see from this plot that the higher predicted values displays heteroskedascity, and so do lower predicted values, which indicates that for some values, the model may not be accounting for other factors.

```{r qq-plot-heteroskedascity}
# Create QQ-plot showing the heteroskedastic error in low and high predicted values
house_sales_98105 %>% 
  fortify(model = house_mod_98105) %>% 
  ...
  labs(x = "Theoretical quantile", 
       ...
       title = "QQ-plot demonstrates heteroskedastic error in low and high predicted values")
  
```

* The QQ-plot above also demonstrates how the empirical quantiles differ from the theoretical quantiles from the data, if the standardized residuals were normally distributed.  

* There are a variety of smoothing algorithms to visualize the smoothed relationship between variables in a scatterplot.  The help documentation from the super smoother (`supsmu`) method suggests performing cross-validation to choose the optimal smoothing approach for each prediction.  

* Partial residual plots can be constructed by summing the predicted value and the residuals.  We can isolate each predictor variable, look at the partial residual plot, and evaluate the contribution of that predictor variable to the overall adjusted sale price of the home. 
 
```{r partial-residuals-plot}
# Calculate partial residuals
terms <- predict(house_mod_98105, type = "terms") %>% 
  data.frame()
partial_resid <- resid(house_mod_98105) + terms %>% 
  data.frame()

# Plot 
partial_resid_df <- data.frame(SqFtTotLiving = house_sales_98105$SqFtTotLiving, 
                               Term = terms$SqFtTotLiving, 
                               Partial_resid = partial_resid$SqFtTotLiving)
partial_resid_df %>% 
  ggplot(aes(SqFtTotLiving, Partial_resid)) + 
  geom_point() +
  geom_smooth(se = TRUE) +
  geom_line(aes(SqFtTotLiving, Term), 
            linetype = "dashed") +
  my_theme + 
  labs(x = "Square feet of total living space", 
       y = "Partial residual", 
       title = "Non-linear behavior between adjusted sale price and square footage")
```

* Put in another way, the partial residuals represent the residual after subtracting the effects from all of the other variables.  For example, looking at the partial residual plot above, we can see that the effect of the square footage of total living space on the response (adjusted sale price) is not consistently linear after removing the effects of all of the other variables.  

## Polynomial and Spline Regression

### Polynomial Regression

* In some cases, the response is not a linear function of the explanatory variables, and the ordinary least squares regression method isn't appropriate to capture all of the variation in the data.  

* In some cases, transformation of the predictors is feasible (e.g., log-transformation to indicate a multiplicative change in the response variable).  

* We can fit a polynomial regression using the `poly()` function in the standard `lm()` function call.

```{r polynomial-function-housing-data}
# Fit a polynomial regression to the housing data, squaring the square footage of living space
house_mod_98105_poly <- lm(AdjSalePrice ~ poly(SqFtTotLiving) + SqFtLot + Bathrooms + 
...
* In other cases, we can use a spline, or essentially a function fit to discrete, fixed points in a predictor variable, called knots.  The function is composed of piecemeal polynomial functions.  

* The spline can be thought of as bending a string that's connecting various beads together.

* The `splines` package can be used to add spline terms in a regression model.  The one we'll use here is the b-spline, called with the `bs()` function.  

* By default, the `bs()` function defines the knots (the fixed points in the predictor variable) as the boundaries, but we can define further knots as well.  In the example of the housing data below, we'll fit a spline term with `SqFtTotLiving` and define the quantiles as the other knots for the curve to move through.  We're also defining that we want to fit a cubic spline, set with the `degree` argument in the `bs()` function call.

```{r spline-regression-sqfttotlinvg}
# Fit a spline term in the SqFtTotLiving variable in the 98105 housing data
# Define the additional knots for the curve to move through
knots <- quantile(house_sales_98105$SqFtTotLiving, 
                  p = c(0.25, 0.5, 0.75))

# Fit the linear spline regression
house_mod_98105_spline <- lm(AdjSalePrice ~ bs(SqFtTotLiving, knots = knots, degree = 3) +
                               SqFtLot + Bedrooms + Bathrooms + BldgGrade, data = house_sales_98105)

# Calculate the partial residuals
term <- predict(house_mod_98105_spline, 
                type = "terms") %>% 
  data.frame()

partial_resid<- resid(house_mod_98105_spline) + term

# Plot the partial residuals along with the actual values
data.frame(SqFtTotLiving = house_sales_98105$SqFtTotLiving, 
           Prediction = term$bs.SqFtTotLiving..knots...knots..degree...3., 
           Partial_resid = partial_resid$bs.SqFtTotLiving..knots...knots..degree...3.) %>% 
  ggplot(aes(SqFtTotLiving, Partial_resid)) +
  geom_point(alpha = 0.65) +
  geom_smooth(se = TRUE, linetype = "dashed") +
  geom_line(aes(SqFtTotLiving, Prediction), color = "red") +
  scale_y_continuous(labels = function(x) format(x, scientific = FALSE, big.mark = ",")) +
  my_theme +
  labs(x = "Square feet, total living space", 
       y = "Partial residuals", 
       title = "Fitted <span style='color:red'><strong>spline</strong></span> regression line relative to a 
       <span style='color:blue'><strong>smooth</strong></span> curve")
```

## Generalized Additive Models

* In some cases, polynomials may not be able to capture the variability in response for a set of predictor variables, and because the spline regression requires defining the knots, it may not be appropriate without prior knowledge.

* [This blogpost] has an excellent description of the similarities and differences between generalized linear models (GLM) and generalized additive models (GAM) and how the latter can be used to understand complex, non-linear relationships between predictors and responses.

* One key difference between GLM and GAM is the latter uses non-parametric functions to define the terms included in the model, meaning that instead of using functions with a small set of parameters, the functions are defined by the data themselves.  This is a more flexible approach when we don't necessarily know what the patterns may look like.

* Regularization helps to prevent overfitting, making a model more "regular" or more generalizable to new, unseen data.

* One of the important features of GAMs is that we can control the smoothing functions to avoid the "wiggly" smooth lines that obscure interpretability in model output, for example, given some prior knowledge that a process may be smooth in nature.

* In situations where the relationship between a predictor and response is non-linear and has a complex shape, like a "hockey-stick" associated with a plateau in the response, fitting different terms in a GLM to try and model that behavior in the response can be difficult.  The GAM framework derives the function of the predictor using the data themselves. 

* I've followed a tutorial from the website linked above as a brief introduction to GAMs.  To start, we'll simulate some data with two variables, `x` (predictor) and `y` (response), where the relationship between these two variables follows a sine function.  We'll also introduce some normally distributed error terms.

```{r simulate-data-gam}
# Create some simulated data where X and Y follow a sine function with normally distributed errors
set.seed(3)
x <- seq(0, 2*pi, 0.1)
z <- sin(x)
y <- z + rnorm(mean = 0, 
...
# Print output
head(df) %>% 
  kable_layout(caption = "Simulated data for GAM tutorial")
```

* We then want to fit the model $y = s_\lambda(x) + e$, where $s_\lambda(x)$ is the smooth function, where the level of "smoothness" is determined by the parameter $\lambda$.  The higher the value of $\lambda$, the more smooth the curve, but we need to strike a balance between a "wiggly" line with $\lambda = 0$, which will generally go through most of the points but miss the overall pattern in the data; versus a completely smooth line.

* Ultimately, the idea is to fit lines with different values of $\lambda$ and then calculate the distance between the smoothed line and the true underlying pattern (the sine curve).

* We can take these concepts to the housing data to fit a GAM using the code below.

```{r gam-housing-data}
# Fit a GAM for the housing 98105 data
house_mod_98105_gam <- gam(AdjSalePrice ~ s(SqFtTotLiving) + SqFtLot + Bathrooms + 
                             Bedrooms + BldgGrade, data = house_sales_98105)

# The 's()' function finds the best knots to "feed" the spline through

# PLot the partial residuals
terms <- predict(house_mod_98105_gam, type = "terms") %>% 
  data.frame()
partial_resid <- resid(house_mod_98105_gam) + terms %>% 
  data.frame()

data.frame(SqFtTotLiving = house_sales_98105$SqFtTotLiving, 
           Prediction = terms$s.SqFtTotLiving., 
           Partial_resid = partial_resid$s.SqFtTotLiving.) %>% 
  ggplot(aes(SqFtTotLiving, Partial_resid)) +
  geom_point(alpha = 0.65) +
  geom_smooth(se = TRUE, linetype = "dashed") +
  geom_line(aes(SqFtTotLiving, Prediction), color = "red") +
  ...
  labs(x = "Square feet, total living space", 
       y = "Partial residual", 
       title = "<span style='color:red'><strong>GAM curve</strong></span> relative to a 
       <span style='color:blue'><strong>smooth curve</strong></span>")
```
